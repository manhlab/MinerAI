...
filename = "data/lock_" + now.strftime("%d-%H%M") + ".csv"
...
SAVE_NETWORK = (
    100  # After this number of episodes, the DQN model is saved for testing later.
)
...
DQNAgent = DQN(INPUTNUM, ACTIONNUM, batch_size = BATCH_SIZE)
...
for episode_i in range(0, N_EPISODE):
    try:
        ...
        if np.mod(episode_i + 1, SAVE_NETWORK) == 0 and train == True:
            DQNAgent.replace_target_network()  # Replace the learning weights for target model with soft replacement
            # Save the DQN model
